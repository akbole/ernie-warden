...
def get_ernie_quote(user, context_type):
    ...
    quotes = {
        "work_low": ["Доверие — это валюта. А ты банкрот.", f"{nickname}, ты потратил энергию на ЭТО? Впрочем, твой выбор.", "Эффективность: 12%. Но ты стараешься."],
        "work_medium": [f"{nickname}... Ты не сдаёшься. Интересно.", "47% пути. Большинство сдались на 30%.", "Продолжай. Я наблюдаю."],
        "work_high": [f"{nickname}, ты помогаешь. Всегда. Даже когда невыгодно.", "Интересно... Ты не похож на остальных.", f"Доверие: {trust}%. Ты близко."],
        "elevator": [f"Этаж {floor}. Половина пути позади. Или впереди?", f"{nickname}... Мало кто доходит так далеко.", "Ты поднимаешься. Я помню каждый твой выбор."],
        "night": [f"{hour:02d}:{datetime.now().minute:02d}. Ты не спишь. Я тоже. Но у меня нет выбора.", f"{nickname}, ты один не спишь на этом этаже. Сейчас."]
    }
    ...
    if context_type == "elevator":
        return quotes["elevator"][floor % len(quotes["elevator"])]
